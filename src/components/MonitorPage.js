...
class MonitorPage extends Component {
  ...
  getAllModularMonitorData = async (hubId, version) => {
    if (version ==3) {
      ...
      var vacants2 = result.data.filter(val => val.sensor_status=='O' && val.hub_id=='6DE7E4D80000')
      var vacants1 = result.data.filter(val => val.sensor_status=='O' && val.hub_id=='AAF814D80000')
      var vacants0 = result.data.filter(val => val.sensor_status=='O' && val.hub_id=='F10414D80000')
      this.setState({
...
client.activate();
  ...
  getMonitorData =  async (hubId, version) => {
    ...
    if (version ==3) {
      ...
      console.log(result.data)
      var vacants = result.data.filter(val => val.sensor_status=='O')
      this.setState({
